...
public class ShiftCalDB {
	...
	private static final String TABLE_CREATE_SHIFT = "CREATE TABLE "
			+ DATABASE_SHIFT_TABLE + " (" + KEY_ROWID
			+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + KEY_SHIFT_NAME
			+ " TEXT, " + KEY_SHIFT_SYMBOL + " TEXT, " + KEY_SHIFT_COLOR
			+ " INT, " + KEY_SHIFT_COMMENT + " TEXT, " + KEY_SHIFT_START_HOURS
			+ " INT, " + KEY_SHIFT_START_MINUTES + " INT, "
			+ KEY_SHIFT_END_HOURS + " INT, " + KEY_SHIFT_END_MINUTES + " INT);";
	...
	public Shift getShiftByID(int id) {
		...
		String[] colList = { KEY_SHIFT_NAME, KEY_SHIFT_SYMBOL, KEY_SHIFT_COLOR,
				KEY_SHIFT_COMMENT, KEY_ROWID, KEY_SHIFT_START_HOURS,
				KEY_SHIFT_START_MINUTES, KEY_SHIFT_END_HOURS,
				KEY_SHIFT_END_MINUTES };

		String whereClause = KEY_ROWID + " = " + Integer.toString(id);

		this.openRead();

		Cursor cursor = db.query(true, DATABASE_SHIFT_TABLE, colList,
		...
		if (cursor.moveToFirst()) {
			...
			ret.symbol = cursor.getString(cursor
					.getColumnIndex(KEY_SHIFT_SYMBOL));
			ret.color = cursor.getInt(cursor.getColumnIndex(KEY_SHIFT_COLOR));
			ret.comments = cursor.getString(cursor
					.getColumnIndex(KEY_SHIFT_COMMENT));
			ret.id = cursor.getInt(cursor.getColumnIndex(KEY_ROWID));
			ret.startTime.minute = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_START_MINUTES));
			ret.startTime.hour = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_START_HOURS));
			ret.endTime.minute = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_END_MINUTES));
			ret.endTime.hour = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_END_HOURS));
	...
	public List<Shift> getAllShifts() {
		...
		Cursor cursor = db.query(true, DATABASE_SHIFT_TABLE, colList, null,
		...
		while (cursor.moveToNext()) {
			...
			sh.startTime.minute = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_START_MINUTES));
			sh.startTime.hour = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_START_HOURS));
			sh.endTime.minute = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_END_MINUTES));
			sh.endTime.hour = cursor.getInt(cursor
					.getColumnIndex(KEY_SHIFT_END_HOURS));
	...
	public Shift getShiftByDate(Date d) {
		...
		if (cursor.moveToFirst()) {
			s = getShiftByID(cursor.getInt(cursor
					.getColumnIndex(KEY_DATES_SHIFTID)));
		} else {
			s = null;
		}
		cursor.close();
